<?php
/**
 * @version		$Id: admin.directory_categories.php 10859 2008-08-30 06:44:09Z willebil $
 * @package		Joomla
...
JTable::addIncludePath(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_directory_categories'.DS.'tables');
...
function showCategories( $section, $option )
{
	...
	. ' g.name AS groupname, u.name AS editor, COUNT( DISTINCT s2.checked_out ) AS checked_out_count,'
	. ' sec.name AS section_name'
	. $content_add
	. ' FROM #__directory_categories AS c'
	. ' LEFT JOIN #__users AS u ON u.id = c.checked_out'
	. ' LEFT JOIN #__groups AS g ON g.id = c.access'
	. ' LEFT JOIN #__'.$table.' AS s2 ON s2.catid = c.id AND s2.checked_out > 0'
	. ' LEFT JOIN #__directory_sections AS sec ON sec.id = c.contact_section'
	. $content_join
	. $where
	. $filter
	. ' AND c.published != -2'
	...
	directory_categories_html::show( $rows, $section, $section_name, $pageNav, $lists, $type );
}
...
function editCategory($edit )
{
	...
	if (!$sections && $type != 'other'
            ...
            && $section != 'com_directory'
			&& $section != 'com_banner') {
		$mainframe->redirect( 'index.php?option=com_directory_categories&section='. $section, JText::_( 'WARNSECTION', true ) );
	}

	$row =& JTable::getInstance( 'directoryCategory' );
	...
	if ( JTable::isCheckedOut($user->get ('id'), $row->checked_out )) {
		...
		$mainframe->redirect( 'index.php?option=com_directory_categories&section='. $row->section, $msg );
	}
	...
	if ( $section == 'com_content' ) {
		...
		// if the section is contact details, make the section list a list of contact_sections
		if ( $section == 'com_contact_details' || $section == 'com_directory' ) {
			if ( $section == 'com_contact_details' ) {
				$lists[ 'section' ] = getSections( $db, $row->contact_section );
			}
			else {
				$lists[ 'section' ] = getSections( $db, $row->contact_section, true );
			}
			$row->section = $section;
			$lists[ 'section' ] .= '<input type="hidden" name="section" value="'. $row->section .'" />';
		}
		else {
			if ( $type == 'other' ) {
	...
	$lists['published'] 		= JHTML::_('select.booleanlist',  'published', 'class="inputbox"', $published );
 	...
 	directory_categories_html::edit( $row, $lists, $redirect );
}
...
function saveCategory()
{
	...
	switch ( JRequest::getCmd('task') )
	{
		...
		case 'apply':
			...
			$mainframe->redirect( 'index.php?option=com_directory_categories&section='. $redirect .'&task=edit&cid[]='. $row->id, $msg );
			break;

		case 'save':
		default:
			$msg = JText::_( 'Category saved' );
			$mainframe->redirect( 'index.php?option=com_directory_categories&section='. $redirect, $msg );
			break;
	}
}

/**
* Deletes one or more categories from the directory_categories table
* @param string The name of the category section
...
function removeCategories( $section, $cid )
{
	...
	if (count( $cid )) {
		$cids = implode( ',', $cid );
		$query = 'DELETE FROM #__directory_categories'
		. ' WHERE id IN ( '.$cids.' )'
		;
		$db->setQuery( $query );
		if (!$db->query()) {
	...
	if (count( $err )) {
		...
		$mainframe->redirect( 'index.php?option=com_directory_categories&section='. $section, $msg );
	}

	$mainframe->redirect( 'index.php?option=com_directory_categories&section='. $section );
}
...
function publishCategories( $section, $cid=null, $publish=1 )
{
	...
	$query = 'UPDATE #__directory_categories'
...
function cancelCategory()
{
	...
	$mainframe->redirect( 'index.php?option=com_directory_categories&section='. $redirect );
}
...
function orderCategory( $uid, $inc )
{
	...
	$mainframe->redirect( 'index.php?option=com_directory_categories'. $section );
}
...
function moveCategorySelect( $option, $cid, $sectionOld )
{
	...
	. ' FROM #__directory_categories AS a'
	. ' WHERE a.id IN ( '.$cids.' )'
	;
	$db->setQuery( $query );
	...
	directory_categories_html::moveCategorySelect( $option, $cid, $SectionList, $items, $sectionOld, $contents, $redirect );
}
...
function moveCategorySave( $cid, $sectionOld )
{
	...
	if (!$sectionMove)
	{
		...
		moveCategorySelect( 'com_directory_categories', $cid, $sectionOld );
	...
	$mainframe->redirect( 'index.php?option=com_directory_categories&section='. $sectionOld );
}
...
function copyCategorySelect( $option, $cid, $sectionOld )
{
	...
	directory_categories_html::copyCategorySelect( $option, $cid, $SectionList, $items, $sectionOld, $contents, $redirect );
}
...
function copyCategorySave( $cid, $sectionOld )
{
	...
	{
		$msg = JText::_('Please select a section from the list');
		copyCategorySelect( 'com_directory_categories', $cid, $sectionOld );
		JError::raiseWarning(500, $msg);
		return;
	}
	...
	$category =& JTable::getInstance( 'directoryCategory' );
	...
	$mainframe->redirect( 'index.php?option=com_directory_categories&section='. $sectionOld, $msg );
}
...
/**
 * Retrieves the different options for contact_sections from the database.
 * @param database object &$db the database to run the query on
 * @param string $key the key of the value that will be selected in the dropdown list
 * @param bool $getDirectory whether or not to retrieve the information from the directory component
 * @return string $list an HTML select list of contact_sections
 */
function getSections( &$db, $key, $getDirectory = false ) {
	$query = getSectionsQuery( $getDirectory );
	$db->setQuery( $query );
	$sections = $db->loadObjectList();
	$list = JHTML::_( 'select.genericlist', $sections, 'contact_section', 'class="inputbox" size="1"', 'id', 'name', $key );
	return $list;
}

/**
 * Builds a query for selecting the contact_sections from the database.
 * @param bool $getDirectory whether or not to retrieve the information from the directory component
 * @return string $query the desired query
 */
function getSectionsQuery( $getDirectory ) {
	$from = $getDirectory ? '#__directory_sections' : '#__contact_sections';
