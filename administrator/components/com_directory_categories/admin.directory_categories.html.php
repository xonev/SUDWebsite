<?php
/**
 * @version		$Id: admin.directory_categories.html.php 10439 2008-06-21 20:34:35Z willebil $
 * @package		Joomla
...
* @subpackage	DirectoryCategories
*/
class directory_categories_html
{
	...
	*/
	function show( &$rows, $section, $section_name, &$page, &$lists, $type )
	{
		...
		?>
		<form action="index.php?option=com_directory_categories&amp;section=<?php echo $section; ?>" method="post" name="adminForm">
		...
		<thead>
			<tr>
				...
				<?php // if these are contact categories, display what section each category is in
				if ( $section == 'com_directory' ): ?>
					<th width="10%">
						<?php echo JHTML::_( 'grid.sort', 'Section', 'c.section_name', @$lists[ 'order_Dir' ], @$lists[ 'order' ] ); ?>
					</th>
				<?php endif; ?>
				<th width="5%">
		...
		for ($i=0, $n=count( $rows ); $i < $n; $i++) {
			...
			$link = 'index.php?option=com_directory_categories&section='. $section .'&task=edit&cid[]='. $row->id .'&type='.$type;
			...
			<tr class="<?php echo "row$k"; ?>">
				...
				<?php // if these are contact categories, display which section each category is in
				if ( $section == 'com_directory' ): ?>
					<td>
		...
		<input type="hidden" name="option" value="com_directory_categories" />
