 ...
 * @subpackage	Contacts
 */
class DirectoryViewContact extends JView
{
	function display($tpl = null)
	{
		...
		$positions = $model->getPositions( $options );
		...
		if (isset( $menu ) && isset($menu->query['view']) && $menu->query['view'] != 'contact'){
			...
			//If there was a category ID in the URL then add the category to the breadcrumb pathway
			if ( $categoryId > 0 ) {
				// iterate through the positions to find the category that we came from
				foreach ( $positions as $position ) {
					if ( $categoryId == $position->catid ) {
						$pathway->addItem( $position->title, 
							JRoute::_( 'index.php?option=com_directory&view=category&catid=' . $position->catid ) );
						break;
					}
				}
			}
