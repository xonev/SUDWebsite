...
/**
 * Directory Component Controller
 *
 * @static
 * @package		Joomla
 ...
 * @since 1.5
 */
class DirectoryController extends JController
{
	/**
	 * Display the view
	 */
	function display()
	{
		...
		switch ($this->getTask())
		{
			//index.php?option=com_directory&task=category&id=0&Itemid=4
			case 'category':
				$viewName	= 'category';
				$layout		= 'default';
				break;
	...
	function submit()
	{
		...
		$link = JRoute::_('index.php?option=com_directory&view=contact&id='.$contact->slug.'&catid='.$contact->catslug, false);
	...
	function vcard()
	{
		...
		JTable::addIncludePath(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_directory'.DS.'tables');
		...
		if ($pparams->get('allow_vcard', 0))
		{
			...
			require_once(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_directory'.DS.'helpers'.DS.'vcard.php');
			$v = new JvCard();
