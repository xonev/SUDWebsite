...
function getOtherCategoryId( $db, $id ) {
	...
	$query = sprintf( 'SELECT id FROM jos_directory_categories WHERE title="%s" AND section="com_directory"', $title );
	$result = $db->query( $query );
	return $result->fetch_object()->id;
}

function updateId( $db, $oldId, $newId ) {
	$query = sprintf( 'UPDATE jos_directory_contacts SET catid=%d WHERE catid=%d', $newId, $oldId );
	$db->query($query);
	echo $db->error;
}
